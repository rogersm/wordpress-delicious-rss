...
function get_deliciousRSS() {
  ...
  // the function can accept up to five parameters, otherwise it uses option panel defaults
  for($i = 0 ; $i < func_num_args(); $i++) {
    $args[] = func_get_arg($i);
  }

  if (!isset($args[0]))
    $userid = stripslashes(get_option('deliciousRSS_userid'));
  else
    $userid = $args[0];

  if (!isset($args[1]))
    $num_items = get_option('deliciousRSS_display_numitems');
  else
    $num_items = $args[1];

  if (!isset($args[2]))
    $tags = transform_tags(get_option('deliciousRSS_tags'));
  else
    $tags = transform_tags($args[2]);

  if (!isset($args[3]))
    $before_link = stripslashes(get_option('deliciousRSS_before'));
  else
    $before_link = $args[3];

  if (!isset($args[4]))
    $after_link = stripslashes(get_option('deliciousRSS_after'));
  else
    $after_link = $args[4];
  ...
  if( ! $rss ) {
    print "<b>del.icio.us is having problems.</b> I'll try again later"
    return FALSE;
  }
...
// Transform "computer, apple ii, tech,, apple, graphics  " into "computer+apple%20+tech+apple+graphics"
// or "computer tech apple graphics  " into "computer+tech+apple+graphics"
// $list_of_tags - a comma separated list of tags
...
function deliciousRSS_subpanel()
{
  ...
  ?>
    ...
    <div class="wrap">
       ...
       <td><input name="display_numitems" id="display_numitems" type="text" value="<?php echo htmlspecialchars(stripslashes(get_option('deliciousRSS_display_numitems')))?>"></td>
       </tr>
       <tr>
